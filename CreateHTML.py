...
for folder in index_missing:
	...
	for i in range(0, len(gif_files)):
		...
		try:
			with codecs.open(txt_file, mode='r') as f:
				subtitle = f.read()
		except IOError:
			print "Unable to read %t" %txt_file
			sys.exit(1)
	...
	try:
		with codecs.open("%s/index.html" %folder, 'w') as f:
			f.write(codecs.BOM_UTF8)
			f.writelines(index)
	except IOError:
		print "Unable to write index.html to %s" %folder
		sys.exit(1)
...
for folder in indexes:
	...
	subtitle = "Show: %s <br>" %folder + "This show contains %i gif(s)."% gif_count + EOL + "Click to open."
...
try:
	...
	print "Unable to write index.html. Aborting!"
	sys.exit(1)
		
try:		
	with codecs.open("style.css", 'w') as f:
		f.write(codecs.BOM_UTF8)
		f.writelines(css_style)
except IOError:
	print "Unable to write style.css. Aborting!"
	sys.exit(1)
